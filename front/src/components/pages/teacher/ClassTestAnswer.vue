...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      try {
        const singleChoiceQuestions = await this.getAllData('single_choice');
        const judgementQuestions = await this.getAllData('judgement');
        const fillinQuestions = await this.getAllData('fillin');
    ...
    async openDatabase() {
      return openDB('ClassTestProblems', 1, {
        upgrade(db) {
          if (!db.objectStoreNames.contains('single_choice')) {
            db.createObjectStore('single_choice', { keyPath: 'id', autoIncrement: true });
          }
          if (!db.objectStoreNames.contains('judgement')) {
            db.createObjectStore('judgement', { keyPath: 'id', autoIncrement: true });
          }
          if (!db.objectStoreNames.contains('fillin')) {
            db.createObjectStore('fillin', { keyPath: 'id', autoIncrement: true });
          }
        }
      });
    },
